...
namespace HeavenStrikeFizz
{
    class Program
    {
        ...
        {
            //Game.Say(Player.Distance(Game.CursorPos).ToString());
            if (Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo)
            {
                combo();
            ...
            if (Eharass)
            {
                ...
                // E2
                {
                    ...
                    if (blockmovebool && target.IsValidTarget() && !target.IsZombie)
                    {
                        ...
                        {
                            Player.IssueOrder(GameObjectOrder.MoveTo, Prediction.GetPrediction(target, 1).UnitPosition);
                        }
                        else
                        {
                            ...
                            {
                                Player.IssueOrder(GameObjectOrder.MoveTo, Prediction.GetPrediction(target2, 1).UnitPosition);
                            }
                            else
                            {
